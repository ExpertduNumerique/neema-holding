...
function ContactRow({ icon, children, href }: { icon: ReactNode; children: ReactNode; href?: string }) {
  const content = (
    <>
      <span
      ...
      <span className="text-gray-700 pt-1.5">{children}</span>
    </>
  );
  return (
    <li className="flex items-start gap-3">
      {href ? (
        <a href={href} className="flex items-start gap-3 hover:text-navy-900 transition-colors">
          {content}
        </a>
      ) : (
        content
    ...
    <>
      <section className="relative section-navy text-white overflow-hidden">
        <div
          ...
          style={{ background: "radial-gradient(circle, rgba(26,102,255,.15), transparent 70%)" }}
          aria-hidden="true"
        />
        ...
        <div className="container-neema grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-4">
            ...
            <ul className="space-y-4">
              <ContactRow icon={<MapPinIcon />}>Koudougou / Ouagadougou, Burkina Faso</ContactRow>
              <ContactRow icon={<PhoneIcon />} href="tel:+22666081896">(+226) 66 08 18 96</ContactRow>
              <ContactRow icon={<PhoneIcon />} href="tel:+22670420566">70 42 05 66</ContactRow>
              <ContactRow icon={<PhoneIcon />} href="tel:+22668733969">68 73 39 69</ContactRow>
              <ContactRow icon={<FacebookIcon />}>Neema Holding International</ContactRow>
            </ul>

            <div className="mt-8 rounded-2xl p-6" style={{ backgroundColor: "#F0F5FF" }}>
              <p className="text-sm font-bold text-navy-900 mb-3">Nos pôles</p>
            ...
            <div className="mt-6 rounded-2xl p-6 flex items-center gap-4" style={{ backgroundColor: "#0B2A66" }}>
              <Image src="/images/neema/logo_holding.png" alt="" width={44} height={44} aria-hidden="true" className="shrink-0" />
              <p className="text-sm font-medium text-white leading-snug">
                « Unir les forces, créer la valeur,{" "}
                <span style={{ color: "#22B863" }}>bâtir l&apos;avenir</span>. »
              </p>
            </div>
          </div>
